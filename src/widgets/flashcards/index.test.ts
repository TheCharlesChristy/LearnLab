// parseProps accept/reject table for `flashcards` (FR-WID-003: every error
...
describe('flashcards parseProps — accept', () => {
  it('accepts a non-empty src', () => {
    expect(parseProps({ src: 'cards/unit1.json' })).toEqual({
      ok: true,
      props: { src: 'cards/unit1.json' },
    });
  });
});

describe('flashcards parseProps — reject (errors name the prop, FR-WID-003)', () => {
...
describe('flashcards def export (FR-WID-001 contract)', () => {
