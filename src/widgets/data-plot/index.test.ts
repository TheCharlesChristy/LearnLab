// parseProps accept/reject table for `data-plot` (FR-WID-003: every error
...
describe('data-plot parseProps — accept', () => {
  it('accepts a non-empty src', () => {
    expect(parseProps({ src: 'data/growth.json' })).toEqual({
      ok: true,
      props: { src: 'data/growth.json' },
    });
  });
});

describe('data-plot parseProps — reject (errors name the prop, FR-WID-003)', () => {
...
describe('data-plot def export (FR-WID-001 contract)', () => {
