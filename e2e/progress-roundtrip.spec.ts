...
// loads the real suite from ./progress-roundtrip.pw.ts.
if (process.env.VITEST) {
  const { test } = await import('vitest');
  test.skip('Playwright e2e suite — run with `npx playwright test` (see e2e/progress-roundtrip.pw.ts)', () => {});
} else {
  await import('./progress-roundtrip.pw');
}
